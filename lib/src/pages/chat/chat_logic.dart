...
class ChatLogic extends GetxController {
  ...
  /// 大于1000为通知类消息
  void markMessageAsRead(int index, Message message, bool visible) async {
    ...
    if (visible && message.contentType! < 1000) {
      ...
      if (isSingleChat) {
        _markC2CMessageAsRead(index, message, visible);
      } else {
        _markGroupMessageAsRead(index, message, visible);
      }
    }
  }

  /// 标记消息为已读
  _markC2CMessageAsRead(int index, Message message, bool visible) async {
  ...
  /// 标记消息为已读
  _markGroupMessageAsRead(int index, Message message, bool visible) async {
    if (!message.isRead! && message.sendID != OpenIM.iMManager.uid) {
      print('mark as read：$index ${message.clientMsgID!} ${message.isRead}');
  ...
  dynamic parseCustomMessage(int index) {
    if (index < 0 || index > messageList.length - 1) return;
    var message = indexOfMessage(index);
    try {
      switch (message.contentType) {
        case MessageType.custom:
  ...
  /// 群消息已读预览
  void viewGroupMessageReadStatus(index) {
    var message = indexOfMessage(index);
  ...
  /// 失败重发
  void failedResend(index) {
    var message = indexOfMessage(index);
    _sendMessage(message, addToUI: false);
  }

  /// 计算这条消息应该被阅读的人数
  int getNeedReadCount(index) {
  ...
  int _calNeedReadCount(Message message) {
    memberList.values.forEach((element) {
      if (element.userID != OpenIM.iMManager.uid) {
        if (element.joinTime! < message.sendTime!) {
